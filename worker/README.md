...
3. Worker 관리 화면 → **Settings → Variables and Secrets**:
   ...
   - **Variable**: `GEMINI_MODEL` = `gemini-2.5-flash` (없어도 코드 기본값 사용. `gemini-2.0-flash`는 계정에 따라
     분당 요청 한도가 0/0으로 배정돼 있을 수 있어 — Google AI Studio "모델별 비율 제한" 화면에서 확인 가능 —
     첫 호출부터 429가 뜰 수 있음. 이미 대시보드에 `GEMINI_MODEL` 변수를 `gemini-2.0-flash`로 등록해뒀다면
     이 값도 `gemini-2.5-flash`로 바꿔줄 것)
   - 저장하면 자동 재배포됨.
