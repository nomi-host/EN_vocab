...
3. Worker 관리 화면 → **Settings → Variables and Secrets**:
   ...
   - **Variable**: `GEMINI_MODEL` = `gemini-2.5-flash-lite` (없어도 코드 기본값 사용. `gemini-2.0-flash`는 계정에 따라
     분당 요청 한도가 0/0으로 배정돼 있을 수 있고, `gemini-2.5-flash`(lite 아닌 버전)는 신규 발급 키에는
     아예 "no longer available to new users"로 막혀있을 수 있음(404). 어떤 모델을 실제로 쓸 수 있는지는
     `https://generativelanguage.googleapis.com/v1beta/models?key=발급받은키`를 브라우저로 열어서
     `generateContent`를 지원하는 stable 모델 이름을 직접 확인하는 게 제일 정확함. 이미 대시보드에
     `GEMINI_MODEL` 변수를 다른 값으로 등록해뒀다면 이 값으로 바꿔줄 것)
   - 저장하면 자동 재배포됨.
