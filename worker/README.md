...
3. Worker 관리 화면 → **Settings → Variables and Secrets**:
   ...
   - **Variable**: `GEMINI_MODEL` = `gemini-3-pro-preview` (★중요★ — 이 값은 **코드보다 대시보드 Variable이 항상
     우선**합니다. 대시보드에 옛날 값이 남아있으면 코드를 아무리 고쳐도 그 값이 계속 쓰이니, 모델 문제가 재발하면
     반드시 여기부터 확인할 것. `gemini-2.0-flash`·`gemini-2.5-flash`·`gemini-2.5-flash-lite` 전부 이 계정(신규
     발급 키)에서는 안 됨 확인함(2.0-flash는 분당 한도 0/0, 나머지 둘은 models.list엔 있어도 실제 호출 시
     404 "no longer available to new users") — 즉 **models.list에 나온다고 실제로 쓸 수 있는 게 아님**,
     2.x 세대 전체가 막혀있는 것으로 보임. 어떤 모델을 실제로 쓸 수 있는지는
     `https://generativelanguage.googleapis.com/v1beta/models?key=발급받은키`를 브라우저로 열어서
     확인하되, 안 되면 이번처럼 3.x 이상 최신 세대부터 시도할 것)
   - 저장하면 자동 재배포됨.
