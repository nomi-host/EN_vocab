...
작성: 2026-07-23 · 상태: **실측 완료 + 선행 조건(5장 분할 로딩) 구현 완료 — 배치 트랙(3장)은 실행 대기** · 선행 문서: `NGSL_IMPORT_PLAN.md`(이 계획에 흡수됨)
...
| 항목 | 배치 수 | 성격 | 권장 모델 |
|---|---|---|---|
| A-2 큐레이션 (14,085개 판정) | ~29 | 가벼움(판정만) | **Haiku 4.5** — 단, 첫 1~2배치를 Sonnet과 하이쿠에 동일하게 주고 판정 일치율 확인 후 확정(불일치 크면 Sonnet 유지). 애매 표시분은 Sonnet 재판정 (2026-07-23 결정) |
...
## 5. 선행 기술 작업 — words.js 분할 로딩 `[구현 완료 — 2026-07-23, Fable]`

배경: 단일 words.js가 20,000개면 ~20MB(gzip ~4.5MB) — 첫 로드·메모리·버전 갱신에 무리. **분할 전에 1만 단어를 편입하면 안 됨** → 완료됐으므로 대량 편입 가능 상태.

구현된 구조 (v0.36.0):
- **코어/청크 2단 로딩**: `merge_cefrj.js`가 words.js(코어 A1~B2, 7,161개)와 `words_c.json`(C1·C2 1,826개) JSON 청크로 분할 출력. 청크 목록은 words.js 끝의 `window.SEED_CHUNKS` 매니페스트 — 확장분(신규 1.1만)이 늘어나면 merge가 청크 파일을 추가하기만 하면 되고 index.html 수정 불필요. 초기 로드 gzip 2.09MB→1.63MB.
- **"타고타고" 보존 원칙**: 청크는 선택 로드가 아니라 **"전부, 나중에" 로드** — index.html의 `loadSeedChunks()`가 렌더와 병렬로 즉시 시작해 SEED에 합류시키므로(보통 부팅 후 1~2초 내) 유의어·반의어·어근 네트워크는 전체 데이터 위에서 동작. 합류 시 파생 인덱스(byWord·SYN_INDEX)는 증분 갱신, 캐시(`_byFlat`·`_seedItemsCache`·`_ltbTaggedCache`)는 무효화 + `seedchunk` 이벤트로 App 리렌더·캐시 재워밍.
- **IndexedDB 캐시 + 버전 정합성**: 청크는 IDB에 `APP_VERSION|파일명` 키로 저장 — 재방문 시 네트워크 없이 합류(오프라인 리로드 실검증 완료), 버전이 바뀌면 키가 달라져 자연 무효화(다른 버전 키는 정리). 구버전 청크가 새 앱에 섞이는 일이 키 구조상 불가능.
- **레벨테스트 게이트**: C1·C2 밴드가 필요한 레벨테스트만 `seedReady`를 기다림("단어 데이터 불러오는 중…" — 부팅 직후 바로 진입한 경우에만 잠깐 노출, 실검증 완료). 학습 세션은 게이트 없음(코어만으로 동작, 합류되면 풀 확장).
- **실패 내성**: 청크 fetch 실패 시 4회 백오프 재시도 후 코어만으로 계속 + `online` 복귀 시 재시도.
- **파이프라인 호환**: words.js만 require하면 코어만 보이므로 전체가 필요한 스크립트는 `pipeline/load_seed.js` 사용(enrich_network/enrich_roots/gen_wordnet_senses/gen_work/expansion 전부 전환 완료 — 분할 전후 실측 수치 동일 확인).

검증(Playwright 실렌더, 2026-07-23): 부팅 SEED 7,161→합류 8,987 · C2 단어(aberration) 검색·상세 시트(senses·유의어 칩) 정상 · IDB 캐시 키 `v0.36.0|words_c.json` · 네트워크 차단 리로드에서 캐시만으로 합류 · 게이트 표시→합류 후 자동 해제 · 콘솔 에러 0.
